...
def kinetic(mass, speed):
  ...
  Returns the Kinetic Energy of that particle,
  in watt-seconds'''
...
def velocity(speed, helm):
  ...
  as a curve-ray phasor representing the arc subtended in 1 second'''
	if helm < MIN_HELM:
		return speed * eul(0)
	else:
...
class Funxion:
    ...
    at x = self.in_val using nearby x-values'''
	...
	def scale_x(self, k):
		'''<Scaling factor>
    Returns Funxion object like this open
    but with x-axis scaled by <k>'''
	...
	def shift(self, k):
		'''<Number>
    Returns Funxion object just like this one,
    except output shifted by <k> units'''
	...
	def shift_x(self, k):
		'''<Number>
    Returerns Funxion object like this one, 
    but x-axis shifted <k> units'''
...
def Sinusoid(w, ph):
  ...
  Returns Funxion object representingsinusoid with equation
  y(x) = sin( <w> x + <ph> )
  '''
...
def Periodic(funx, period):
	'''<Python function> representing math function, <Period> of periodic function
  Returns Funxion object representing periodic function with period <period>
  and is equal to y(x) = <funx>(x) on domain [0, period]'''
...
def Quadratic(a, b, c):
	'''<Coefficient of x^2>, <... of x>, <... of 1>
  Returns FUnxion object representing equation
  y( x )  =  <a> x^2  +  <b> x  +  <c>'''
...
def Heav(x_trans):
	'''<Point of transition> om independent axis, from y =0 to y = 1
...
def Pulse(x_i, x_f):
	'''<Beginning> , <End> of domain where y = 1
  Returns Funxion object representing rectangular pulse function
  y(x) = u(x - <x_i>) - u(x - <x_f>)'''
	return SumFunxion([Heav(x_i), Heav(x_f).scale(-1)])
...
class Obstacle:
	...
	def ray_length(self, theta):
		'''<Angle> in radians
    Returns distance from centerpoint to border of this Obstacle 
    at angle <theta> radians rightward of angle(self.orientation)'''
		th = theta % (2 * PI)
		return self.funxion.funx(th)

	def ray_at(self, theta):
		'''<Angle> in radians rightward of self.orientation
    Returns phasor representing displacement
    between centerpoint and border of this Obstacle at that angle'''
		return self.ray_length(theta) * eul(theta)
	...
	def move_curve(self, ang, kurv_r):
		'''<Angle> subtended by curve, <Curvature Radius> of curve_ray
    Readjusts this Obstacle's instance variables as if it has moved along a curve 
    described by  curve_ray(<theta>, <kurv_r>)'''
		r = curve_ray(ang, kurv_r)
		self.move(r)
		self.turn(theta)

	def copy(self):
		'''Returns copy of this Obstacle objects'''
		obst = Obstacle(self.funxion.copy(), self.position)
...
def Rectangle(shape_length, shape_width):
	'''<Length of rectangle> in direction parallel to self.orientation, <Width of rectangle> in direction normal to self.orientation
...
class Car:
	def __init__(self, mass, pwr_drive, energy):
		...
		self.pos, self.delta_pos = 0 * eul(0), 0 * eul(
		    0)  #Position, change in position, stored as a phasor
		...
		self.speed = (2.0 * self.energy / self.mass)**(
		    0.5)  #speed, from kinetic energy
		self.pwr_grav = GRAV * sin(
		    self.
		    tilt) * self.mass * self.speed  #mechanical power due to gravity
		...
		self.is_curved = abs(
		    self.helm) > MIN_HELM  #Boolean true if car's path is curved
	...
	def friction(self, m_k):
	  '''<Coefficient of Kinetic Friction> in SI units
    Returns kinetic power in watts due to kinetic friction on this Car
    given coefficient of static friction <m_k>
    using the simplified kinetic friction model taught in physics
     '''
	  weight = self.pwr_grav / self.speed
	  force = m_k * weight
	  return -1 * abs( force * self.speed )
    ...
    Changes the instance variables to model a small amount of motion in the car'''
		...
		if not self.rev:
			...
			self.pos -= self.delta_pos
			#EXPERIMENTAL CODE SECTION
			self.shape.move(self.delta_pos * eul(PI))
			self.shape.turn(-1 * 2 * angle(self.delta_pos))
			#END EXPERIMENTAL CODE SECTION
    ...
    so other code can execute at the same time'''
		thr = threading.Thread(
		    target=self.travel_raw, name="", args=(dur, delta_t))
		thr.start()

	def travel_drive_raw(self, dur_t, drivefunxion):
		'''<Amount of time> in seconds of simulated time, <Funxion object> prepresenting self.pwr_drive as a funciton of simulated time
    Simulates the motion of the car for <dur_t> seconds, 
    if self.pwr_drive is governed by drivefunxion( self.t_now )'''
		n = int(dur_t // self.t_poll)
		for i in range(n):
			drivefunxion.feed(self.t_now)
			self.pwr_drive = drivefunxion.out_val
			self.sweep()
	...
	def travel_drive(self, dur_t, drivefunxion):
		'''<Amount of time> in seconds of simulated time, <Funxion object> representing self.pwr_drive vs. simulated time   
    Threaded version of Car.travel_drive_raw( float, Funxion )'''
		thr = threading.Thread(target=self.travel_drive_raw, name="", args=(dur_t, drivefunxion))
		thr.start()

	def travel_other_raw(self, dur_t, drivefunxion, otherfunxion):
		'''<Amount of time> in simulated time, <FUnxion> representing self.pwr_drive as function of time, <Funxion> representing self.pwr_other as function of kinetic energy  
    SImulates motion of car with all power sources accounted for by sone function or Funxion
    WARNING: drivefunxion is a function of time, while otherfunxion is a function of this Car's kinetic energy'''
		n = int(dur_t // self.t_poll)
		for i in range(n):
			otherfunxion.feed(self.energy)
			self.pwr_other = otherfunxion.out_val
	...
	def travel_other(self, dur_t, drivefunxion, otherfunxion):
		'''<Duration> in simulated time, <Funxion object> representing self.pwr_drive vs. simulated time, <Funxion object> representing self.pwr_other vs. kinetic energy
    Threaded version of Car.travel_other_raw'''
		thr = threading.Thread(target=self.travel_other_raw, name="", args=(dur_t, drivefunxion, otherfunxion))
		thr.start()
    ...
    of this Car object'''
		strega = "self.delta_tau = " + str(
		    self.delta_tau) + "  seconds of processor time\n"
		strega += "self.t_poll = " + str(
		    self.t_poll) + "  seconds of simulated time\n"
		strega += "self.t_now = " + str(
		    self.t_now) + "  seconds of simulated time\n\n\n"
		return strega
    ...
    of this Car object'''
		...
		strega += "self.is_curved = " + str(int(
		    self.is_curved)) + "  boolean\n\n\n"
		return strega

	def tell_motion_raw(self, durn_tau, d_tau):
		#UNTESTED
	...
	def tell_motion(self, durn_tau, d_tau):
    ...
    but in a threaded manner'''
		thr = threading.Thread(target=self.tell_motion_raw, name="", args=(durn_tau, d_tau))
		thr.start()
	...
	def tostring(self):
		...
		strega += "self.t_now = " + str(self.t_now) + "  seconds\nself.frix_coeff = "+str(self.frix_coeff)+"  unitless\nself.shape = "+str(self.shape)+"\n"
...
class Land:
	...
	def sweep(self):
		...
		self.size = min(len(self.tiltvec), len(self.helmvec))
		frix = 0 + self.frixvec[-1]
		for i in range(len(self.frixvec), self.size):
			self.frixvec.append(frix)
		for i in range(self.size):
			ang = self.helmvec[i] * self.delta_space
			if abs(ang) < MIN_HELM:
				self.phasorvec.append(self.delta_space * eul(0))
	...
	def tostring(self):
		...
		strega += "self.carspacevec = " + str(self.carspacevec) + "\n"
...
### TESTING SECTION
cam_mass = 1500
camry = Car(cam_mass, .1*kinetic(cam_mass, 55*MPH), kinetic(cam_mass, 25*MPH))
camry.rename("camry")
camry.t_poll = 0.1
camry.frix_coeff = 0.17
print(camry.tostring())


otherfunxion = Konstant(1000) 

drivefunxion = Linear(-40000, 40000)

#camry.travel_other(1.1, drivefunxion, otherfunxion)
#camry.tell_motion(1.1 * camry.delta_tau / camry.t_poll, camry.delta_tau)
for i in range(5):
  camry.go_other(drivefunxion, otherfunxion)
